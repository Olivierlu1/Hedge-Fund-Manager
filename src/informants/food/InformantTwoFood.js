...
import { hireFoodTwo } from '../../actions/foods';
...
class InformantTwoFood extends Component {
  ...
  onHire = () => {
    if (this.state.price > this.props.stats.netWorth || this.props.food.two.willHire == true) {
      this.setState({ error: true });
    } else {
      this.props.subtractNetWorth(this.state.price);
      this.props.hireFoodTwo();
      this.props.updateNetCash(this.props.stats.netCash - this.state.price);
      this.props.addEvent({ title: `Hired Irma`, description: "Hired Food Informant Level 2"});
  ...
  renderInsiderInformation() {
    ...
    if (this.props.food.two.isHired == true) {
      message = defaultMessage;
    } else if (this.props.food.two.willHire == true) {
      message = hiredMessage;
    } else {
      message = notHiredMessage;
    }

    const holder = events[index].random <= 0.2 && this.props.food.two.isHired && round < events[index].eventRound ? events[index].news : message;
    let temp;
    if (events[index].random <= 0.2 && this.props.food.two.isHired && round >= events[index].eventRound) {
    ...
    return (
      <Container style={{ marginTop: "20px" }}>
        ...
        <p>{temp}</p>
        <br /><br /><br />
        <Button basic color="teal" onClick={this.onHire} disabled={this.props.food.two.willHire}>
            Hire
        </Button>
      </Container>
    );
  ...
  render() {
    return (
      <div>
        ...
        {
          this.props.food.two.willHire &&
          <Message positive header="Successfully Hired Irma Ola" />
        }
        <Grid>
            <Grid.Row>
              <Grid.Column width={8}>
...
export default connect(
  mapStateToProps,
  { hireFoodTwo, subtractNetWorth, addEvent, insiderHired, updateNetCash }
)(InformantTwoFood);